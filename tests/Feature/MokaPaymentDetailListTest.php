...
it('can get payment detail list successfully with paymentId', function ($mockResponse): void {
    Http::fake([
...
it('can get payment detail list successfully with otherTrxCode', function ($mockResponse): void {
    Http::fake([
        'https://service.refmoka.com/PaymentDealer/GetDealerPaymentTrxDetailList' => Http::response($mockResponse),
    ]);
...
it('throws exception when payment detail list fails', function ($mockResponse): void {
    Http::fake([
        'https://service.refmoka.com/PaymentDealer/GetDealerPaymentTrxDetailList' => Http::response($mockResponse),
    ]);

    expect(static fn () => Moka::paymentDetailList()->get('1170'))
...
it('throws exception when no parameters are provided', function (): void {
    expect(static fn () => Moka::paymentDetailList()->get(null, null))
        ->toThrow(function (MokaPaymentDetailListException $exception): void {
